...
class note {
		...
		if (note_count == 12 && hide_fingering == false){
			...
			for(let i=0;i<8;i++){
				...
				if(state == 0 || state == 2){ 
					...
					if(i > 0 ) shift = shift_angle
					arc(this.mid, this.y + this.h*0.0625 + this.h*0.125*i, U*(0.45 + inc2), U*(0.45 + inc2), state*HALF_PI + shift, TAU + shift)
					pop()
				}
			}
				...
				push()
					...
					if(this.scale_index < chart.notes.length){
						...
						for(i=I;i--;){
							fill(0,0,35*i,10)
							// fill(85*i)
							rect(this.mid,v_pos + v_spacing*(i-I), this.mid + this.w*0.5 - m1,v_pos + v_spacing*(i-I+1))
						}
					}
				pop()
			}
	...
	generate_engraving(draw_staff=true, ghost_note=false){
		...
		let transparency = ghost_note? 65 : 255
